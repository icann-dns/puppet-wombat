...
class wombat::compactor (
  ...
  Boolean                             $raw_pcap                 = false,
  Boolean                             $ignored_pcap             = true,
  Array[Wombat::Capture_data]         $capture_data             = ['all'],
  Array[Stdlib::IP::Address]          $ip_addresses             = [$facts['networking']['ip']],
  Optional[Wombat::Listen_interfaces] $listen_interfaces        = undef,
  Optional[String]                    $filter                   = undef,
  ENUM['gzip','xz']                   $compression              = 'xz',
  Integer[0,9]                        $compression_level        = 2,
  ENUM['gzip','xz']                   $pcap_compression         = 'xz',
  Integer[0,9]                        $pcap_compression_level   = 2,
  Array[Integer[0,4096]]              $vlan_id                  = [0],
  Optional[Stdlib::Absolutepath]      $dnstap_socket            = undef,
  String                              $dnstap_socket_owner      = 'root',
  String                              $dnstap_socket_group      = 'root',
  ENUM['all','user','group']          $dnstap_socket_write      = 'group',
  String                              $package                  = 'dns-stats-compactor',
  Stdlib::Absolutepath                $conf_dir                 = '/etc/dns-stats-compactor',
  Stdlib::Absolutepath                $tools                    = '/usr/local/bin',
  String                              $service                  = 'dns-stats-compactor',
  Boolean                             $enable                   = true,
  Integer[0,3600]                     $log_network_stats_period = 0,
  Boolean                             $log_file_handling        = false,
  Boolean                             $sampling_enable          = false,
  Integer[1,100]                      $sampling_threshold       = 10,
  Integer[2,1000]                     $sampling_rate            = 10,
  Integer[10,3600]                    $sampling_time            = 100,
